let apiProjectApi = new TempApi.ProjectApi();import TempApi from '../src/index';document.getElementById('idts').onclick = (event) => {
    ...
    event.preventDefault();
    let projectId = window.location.pathname.replace('/updateOverview/','');
      if(projectId === '/updateOverview' || projectId === ''){
        const storedData = window.localStorage.getItem('data');
        const newMap = new Map(JSON.parse(storedData));
        newMap.forEach( (value, key) => {
          if(document.getElementById(key).contains(document.getElementById('i6ivem')) === true){
            projectId = value._id;
          }
        })
      }
   apiProjectApi.deleteproject( projectId, (error, data, response) => { if (error) {console.error(error);} else { console.log('API called successfully.');{  location.href= '/HomePage' ;}}});};document.getElementById('ir7zdp').onclick = (event) => {
    event.preventDefault();
